...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 w-full z-50 bg-[#fff] text-black shadow-lg">
      ...
      <div className="hidden lg:flex justify-between px-8 lg:px-20 py-8 items-center md:px-20">
        ...
        <a href="https://github.com/Isbahfalaqiy" target="_blank" rel="noopener noreferrer">
          <FaGithub className="text-3xl hover:text-[#C5D86D] relative left-[7px]" />
        </a>
      </div>
      ...
      {isMenuOpen && (
        <div className="flex flex-col gap-6 items-center py-4 bg-white text-black">
          <Link to="/" onClick={() => handleClick("Home")} className={`hover:text-[#C5D86D] ${activeLink === "Home" ? "border-b-4 border-[#C5D86D] text-[#C5D86D]" : ""}`}>
            Home
          </Link>
          <Link to="/About" onClick={() => handleClick("About")} className={`hover:text-[#C5D86D] ${activeLink === "About" ? "border-b-4 border-[#C5D86D] text-[#C5D86D]" : ""}`}>
